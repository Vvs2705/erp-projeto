...
async def get_trial_balance(
    ...
    try:
        report = await ReportingService.get_trial_balance(db, tenant_id, start_date, end_date)
        return report
...
async def get_income_statement(
    ...
    try:
        report = await ReportingService.get_income_statement(db, tenant_id, start_date, end_date)
        return report
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_ageing(
    ...
    reference_date: date = Query(..., description="Reference date for calculating overdue days"),
    db: AsyncSession = Depends(get_db),
    tenant_and_user: tuple[uuid.UUID, uuid.UUID] = Depends(get_current_tenant_and_user)
):
    tenant_id, _ = tenant_and_user
    ...
    try:
        report = await ReportingService.get_ageing_report(db, tenant_id, ageing_type_upper, reference_date)
        return report
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
