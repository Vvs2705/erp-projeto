...
class SalesQuotation(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('draft', 'approved', 'rejected')", name="chk_sales_quotations_status"),
        CheckConstraint("cnpj ~ '^[A-Z0-9]{14}$'", name="chk_sales_quotations_cnpj_format"),
...
class SalesOrder(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('draft', 'approved', 'dispatched', 'cancelled')", name="chk_sales_orders_status"),
...
class SalesOrderItem(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("quantity_dispatched >= 0", name="chk_sales_order_items_quantity_dispatched"),
    )
    ...
    quantity_dispatched: Mapped[Decimal] = mapped_column(Numeric(18, 4), default=Decimal("0.0000"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
