...
from sqlalchemy import String, Numeric, DateTime, Date, ForeignKey, CheckConstraint, UniqueConstraint
...
class Account(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("type IN ('asset', 'liability', 'equity', 'revenue', 'expense')", name="chk_accounts_type"),
    )
    ...
    parent_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("accounts.id", ondelete="RESTRICT"), nullable=True)
...
class FiscalPeriod(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('open', 'locked', 'closed')", name="chk_fiscal_periods_status"),
...
class Journal(Base):
    ...
    # Relationships
    entries: Mapped[list["JournalEntry"]] = relationship(back_populates="journal", cascade="all, delete-orphan")
...
class JournalLine(Base):
    ...
    __table_args__ = (
        CheckConstraint("direction IN ('DEBIT', 'CREDIT')", name="chk_journal_lines_direction"),
...
class Bill(Base):
    __tablename__ = "bills"
    __table_args__ = (
        CheckConstraint("status IN ('pending', 'partially_paid', 'paid', 'cancelled')", name="chk_bills_status"),
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    legal_entity_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("legal_entities.id", ondelete="RESTRICT"), nullable=False)
...
class BillPayment(Base):
    ...
    __table_args__ = (
        CheckConstraint("amount > 0", name="chk_bill_payments_amount"),
    )
    ...
    bank_account_info: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
...
class Invoice(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('pending', 'partially_paid', 'paid', 'cancelled')", name="chk_invoices_status"),
...
class Partner(Base):
    ...
    __table_args__ = (
        CheckConstraint("type IN ('customer', 'supplier', 'both')", name="chk_partners_type"),
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    cnpj: Mapped[str] = mapped_column(String(14), nullable=False)
    type: Mapped[str] = mapped_column(String(50), nullable=False)  # 'customer', 'supplier', 'both'
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
...
class BankTransaction(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "fitid", name="uq_bank_transactions_fitid_tenant"),
    )
